%% Data import
function [Rs, Ls, Rl, Ce, Rm, Cm, Lm, z0_real] = S11_Fitting_RLC(file_name)
    Rs_vector = zeros(1,7);
    Ls_vector = zeros(1,7);
    Rp_vector = zeros(1,7);
    Cp_vector = zeros(1,7);
    ...
    for file_iter = 1:1
        % file_idx = 0;
        % file_name = strcat('B', num2str(file_iter));
        % file_name = strcat(file_name, '.s2p');
        disp(file_name);
        listing(:,1) = dir(file_name);
        N = length(listing(:,1))
         for i = 1:1
            for k = 1:1
        ...
        for m = 1:length(inc)
            ...
            frs = 0.71 * 2 * pi;     % start of the resonance frequency (GHz)
            fre = 0.73 * 2 * pi;     % end of the resonance (GHz)
            ffs = 0.3 * 2 * pi;     % start of the fitting frequencyng frequency (GHz)
            ffe = 1.5 * 2 * pi;     % end of the fitting (GHz)
            ...
            [Rs,Ls,Rl,Ce,Z0f, z0_real] = Z0_fit(0,freq,Z0,frs,fre,ffs,ffe,Rsf0,Lsf0,Rlf0,Clf0,1.5) ;
            Rsp(m) = Rs;
            Lsp(m) = Ls;
            Rlp(m) = Rl;
            Cep(m) = Ce;
            ...
            % plot real/imag of Z0
            figure('Position',[50 50 600 450],'Color',[1 1 1])
            subplot(2,1,1)
            plot(freq_plot',real(Z0),'LineWidth',1.5,'Color','#FF4500')
            ylabel('Z0.Real', 'FontSize',16,'FontWeight','bold')
            xlim([0.71,0.735])
            grid on
            % hold on
            subplot(2,1,2)
            plot(freq_plot',imag(Z0),'LineWidth', 1.5,'Color','#0000CD')
            ylabel('Z0.Imag', 'FontSize',16,'FontWeight','bold')
            xlim([0.71,0.735])
            grid on
            %% Ya fitting
            freq_rad = freq * 2 * pi;
