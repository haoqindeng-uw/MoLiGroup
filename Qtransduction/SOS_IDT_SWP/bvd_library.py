...
class BVD_Model:
    def __init__(self):
        ...
        self.Z_org = None
        self.power = None
        self.power_absorbed = None

    def load_directory(self, directory, db=False):
        power_list = []
        for filename in sorted(os.listdir(directory)):
            file = os.path.join(directory, filename)
            if file.endswith('10.s2p') or file.endswith('dep.s2p'):
                continue
            print(file)
            self.load_file(file, normalize=True)
            power = self.power
            if db is True:
    ...
    def load_file(self, file_name, normalize=True, s='s22'):
        '''
        ...
        self.power = np.min(np.absolute(self.s11_pol))
        self.power_absorbed = 1 - self.power
        # self.powerdB = 10*np.log10(self.power)
        # print(self.powerdB)
        return self.power_absorbed

    def plot_s11(self, freq_unit='GHz', method='python', figsize=(12, 4), figscale=1, db=False):
        '''
        ...
        if db is False:
            ...
            plt.plot(self.freq / scale, 10*np.log(np.absolute(self.s11_pol)))
        # plt.plot(self.freq / scale, 10*np.log10(np.absolute(self.s11_pol)))
        plt.xlabel(x_label)
        plt.ylabel('|S11|')
        plt.subplot2grid((2, 3), (0, 2))
        # plt.plot(self.freq / scale, self.Z_org.real)
        plt.plot(self.freq / scale, self.s11_pol.real)
        plt.xlabel(x_label)
        plt.ylabel('Real(S11)')
        plt.subplot2grid((2, 3), (1, 2))
        # plt.plot(self.freq / scale, self.Z_org.imag)
        plt.plot(self.freq / scale, np.arctan2(self.s11_pol.imag, self.s11_pol.real))
        plt.xlabel(x_label)
